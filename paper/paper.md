...
Existing ecosystem tools like PsychoPy or Psychtoolbox focus primarily on *runtime* stimulus presentation. While powerful, they often couple generation with the display context (requiring specific window backends) and can be complex to automate for large-scale batch generation. CogStim fills a distinct niche by decoupling generation from presentation. By producing standard, static assets (offline generation), it ensures that the resulting stimuli are mathematically reproducible and portable to any experimental framework (web-based, desktop, or mobile) or machine learning pipeline, a flexibility not easily achieved by patching existing runtime-focused libraries.

# Research impact statement

CogStim has demonstrated its utility in diverse research contexts, bridging behavioral science and computational modeling. It has been used to generate stimuli for psychometric assessments in educational settings [@correig-fraga_development_2024] [@correig-fraga_interplay_2025] and to create synthetic datasets for evaluating visual computation models in neuroscience [@correig-fraga_structure_nodate]. These applications validate the library's capability to support both traditional psychological experiments and modern data-driven approaches.
